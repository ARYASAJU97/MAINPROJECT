...
def patienthome(request):
    ...
    return render(request,"patient/index.html",{"pid":pid})
...
def viewpatientsbydoc(request):
    ...
    data=c.fetchall()
    ui="select patientbooking.*,patientreg.*,mri.* from patientbooking inner join patientreg on patientbooking.pid=patientreg.pid join mri on patientreg.pid=mri.pid where patientbooking.did='"+str(request.session['docid'])+"' and patientbooking.status='mri'"
    print(ui)
    c.execute(ui)
    data1=c.fetchall()
    return render(request,"doctor/viewpatients.html",{"data":data,"data1":data1})
...
def patientbooking(request):
    ...
    c.execute("select * from doctorreg where specification='"+specification+"'")
    data=c.fetchall()
    print(data)
    if request.POST:
        ...
        print(type(a))
        msg=a
    return render(request,"patient/patientbooking.html",{"data":data,"msg":msg,"pid":pid})
...
def viewmrirequest(request):
    ...
    c.execute("select mri.*,patientreg.*,doctorreg.* from mri join patientreg on mri.pid=patientreg.pid join doctorreg on doctorreg.did=mri.did where mri.status='requested' and mri.image=''")
    data=c.fetchall()
    print(data)
